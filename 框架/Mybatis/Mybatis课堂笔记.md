...
- 接口实现类由原来的UserDaoImpl转变为一个 Mapper配置文件.
  ...
  <!--select查询语句,resultType返回一个，resultMap返回多个-->
     <select id="getUserList" resultType="com.kuang.pojo.User">//别写集合，写泛型
...
3. 方法名不对
4. 返回类型不对
5. Maven导出资源问题：在pom.xml文件中配置build

Mybatis三个核心接口：SqlSessionFactoryBuilder (一旦创建SqlSessionFactory，就不再需要) -> SqlSessionFactory(运行期间一直存在) -> SqlSession(不是线程安全的，因此不能被共享)
...
只有一个基本类型参数的情况下，可以直接在sql中取到！【xml中可不写parameterType】
...
Mybatis默认的事务管理器就是 JDBC  ， 连接池 ： POOLED，池：用完可以回收，使web响应更快。
...
一些常见的 Java 类型内建的类型别名：它们都是不区分大小写的，注意，为了应对原始类型的命名重复，采取了特殊的命名风格：基本类型加_（int-> _ int），包装类型用其基本类型（Integer->int），都是小写。
...
- 连接到连接池的一个请求！每个线程都应该有它自己的 SqlSession 实例。
...
where子条件满足时，可智能决定and or等是否保留。

set可前置set关键字，可智能决定逗号是否保留。
